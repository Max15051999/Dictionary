...
GET_WORDS_OF_CONCRETE_LANG_INFO_FOR_GUESS_GAME = f""" SELECT original, translate, transcription, lang, date_to_add, is_forgotten, id FROM {config.TABLE_NAME} WHERE lang = ?; """

SET_ALL_WORDS_IS_FORGOTTEN_NO = f""" UPDATE {config.TABLE_NAME} SET is_forgotten = false WHERE lang = ?; """

SET_IS_FORGOTTEN_YES = f""" UPDATE {config.TABLE_NAME} SET is_forgotten = true WHERE id = ?; """